    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            let cond_id = ssa
                ...
                .iter()
                .find_map(|b| {
                    if let SsaTerminator::CBranch { cond, .. } = b.terminator {
                        Some(cond)
                    } else {
                        None
                    }
            ...
            assert_eq!(
                left_off, 8,
                "SLess left operand must be RCX (offset 8), got offset {}",
                left_off
            );
            assert_eq!(
                right_off, 0,
                "SLess right operand must be RAX (offset 0), got offset {}",
                right_off
            );
            ...
            assert!(
                !FLAG_OFFSETS.contains(&left_off),
                "left operand is a flag register"
            );
            assert!(
                !FLAG_OFFSETS.contains(&right_off),
                "right operand is a flag register"
            );
        })
        .expect("thread spawn");
        ...
        .spawn(|| {
            ...
            let (eq_of_sf_id, taken, fallthrough) = {
                if let SsaTerminator::CBranch {
                    ...
                    taken,
                    fallthrough,
                } = ssa.blocks[cbranch_block].terminator
                {
                    let eq_id = if let Expr::BinOp(BinOpKind::BoolAnd, _l, r) =
                        ssa.vars[cond.0 as usize].expr
                    {
                        r
                    } else {
                        panic!(
                            "expected BoolAnd before fold, got {:?}",
                            ssa.vars[cond.0 as usize].expr
                        )
                    };
            ...
            let mismatched_neq =
                ssa.new_var(template_varnode, Expr::BinOp(BinOpKind::NotEq, c0, c1), 1);
            let mismatched_cond = ssa.new_var(
                template_varnode,
                Expr::BinOp(BinOpKind::BoolAnd, mismatched_neq, eq_of_sf_id),
                1,
            );
            ssa.blocks[cbranch_block].terminator = SsaTerminator::CBranch {
                cond: mismatched_cond,
                taken,
                fallthrough,
            };
            ...
            if let SsaTerminator::CBranch { cond, .. } = &ssa.blocks[cbranch_block].terminator {
                ...
                assert!(
                    !matches!(
                        ...
                        Expr::BinOp(BinOpKind::SLess, _, _)
                    ),
...
fn jg_integration_positive() {
    ...
    let data = match std::fs::read(path) {
        Ok(d) => d,
        Err(_) => {
            eprintln!("skipping jg_integration_positive: fixture not found");
            return;
        }
    };
    ...
    let off = match file_off {
        Some(o) => o,
        None => {
            eprintln!("skipping: VA not in any section");
            return;
        }
    };
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            assert!(
                ...
                out.lines()
                    .filter(|l| l.contains("OF") || l.contains("SF"))
                    .collect::<Vec<_>>()
                    .join("\n")
            );
