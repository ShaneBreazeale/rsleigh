...
fn demangle_name(name: &str) -> String {
    ...
    if let Ok(sym) = cpp_demangle::Symbol::new(name.as_bytes()) {
        let Ok(demangled) = sym.demangle() else {
            return clean;
        };
...
fn demangle_swift(name: &str) -> Option<String> {
    ...
    let (module, rest) =
        if s.starts_with('s') && !s.as_bytes().get(0).map_or(false, |b| b.is_ascii_digit()) {
    ...
    if let Some((class_name, after_class)) = parse_swift_identifier(rest) {
        if after_class.starts_with('C') {
            ...
            if let Some((prop_name, after_prop)) = parse_swift_identifier(after_c) {
                ...
                if after_prop.ends_with('F')
                    || after_prop.contains("Tq")
                    || after_prop.contains("Tj")
                {
        ...
        if after_class.ends_with('F')
            || after_class.contains("yS")
            || after_class.contains("ySb")
            || after_class.contains("ySi")
            || after_class.contains("ySS")
            || after_class.contains("ySd")
        {
            return Some(class_name.to_string());
        }
    }
    ...
    if module == "s" {
        ...
        if let Some((method, after_method)) = parse_swift_identifier(after_type) {
            if after_method.contains("vg") {
                return Some(format!("{}.{}.getter", type_name, method));
            }
            if after_method.contains("vs") {
                return Some(format!("{}.{}.setter", type_name, method));
            }
            if after_method.contains("vM") {
                return Some(format!("{}.{}.modify", type_name, method));
            }
            if after_method.contains("Tj") {
...
fn resolve_elf(elf: &goblin::elf::Elf, binary: &[u8], map: &mut HashMap<u64, String>) {
    ...
    for sh in &elf.section_headers {
        ...
        if name == ".plt" || name == ".plt.got" || name == ".plt.sec" {
            ...
            for i in 1..n_entries.min(10000) {
                // cap at 10K entries for safety
                let Some(stub_addr) =
                    plt_start.checked_add(i.checked_mul(entry_size).unwrap_or(u64::MAX))
                else {
                    break;
                };
                let Some(off) = file_off.checked_add((i * entry_size) as usize) else {
                    break;
                };
        ...
        if let Some(sym) = elf.dynsyms.get(reloc.r_sym) {
            if let Some(name) = elf.dynstrtab.get_at(sym.st_name) {
                if !name.is_empty() && reloc.r_offset != 0 {
                    map.entry(reloc.r_offset)
                        .or_insert_with(|| demangle_name(name));
                }
            }
        }
    }
            ...
            && sym.st_value != 0
        {
            if let Some(name) = elf.strtab.get_at(sym.st_name) {
                if !name.is_empty() && !name.starts_with("FUN_") {
                    ...
                    if clean == "__TMC_END__"
                        || clean.starts_with("_ITM_")
                        || clean == "_IO_stdin_used"
                        || clean == "completed.0"
                    {
                        continue;
                    }
                    // Don't overwrite existing names from dynsyms (stdin, stdout, etc.)
                    map.entry(sym.st_value)
                        .or_insert_with(|| demangle_name(name));
                }
            }
        }
...
fn resolve_macho(macho: &goblin::mach::MachO, binary: &[u8], map: &mut HashMap<u64, String>) {
    ...
    if indirect_symoff == 0 || n_indirect_syms == 0 {
        return;
    }
        ...
        if let goblin::mach::load_command::CommandVariant::Segment64(seg) = &lc.command {
            ...
            for i in 0..seg.nsects {
                ...
                let addr = u64::from_le_bytes(
                    binary[sect_off + 32..sect_off + 40]
                        .try_into()
                        .unwrap_or([0; 8]),
                );
                let size = u64::from_le_bytes(
                    binary[sect_off + 40..sect_off + 48]
                        .try_into()
                        .unwrap_or([0; 8]),
                );
                let offset = u32::from_le_bytes(
                    binary[sect_off + 48..sect_off + 52]
                        .try_into()
                        .unwrap_or([0; 4]),
                );
                let reserved1 = u32::from_le_bytes(
                    binary[sect_off + 60..sect_off + 64]
                        .try_into()
                        .unwrap_or([0; 4]),
                );
    ...
    let n_stubs = if stub_size > 0 {
        stubs.size / stub_size
    } else {
        0
    };
...
fn resolve_pe(pe: &goblin::pe::PE, binary: &[u8], map: &mut HashMap<u64, String>) {
    ...
    let ordinal_flag: u64 = if pe.is_64 {
        0x8000000000000000
    } else {
        0x80000000
    };
        ...
        for entry in &import_data.import_data {
            ...
            if let Some(ref ilt) = entry.import_lookup_table {
                ...
                for (i, lookup) in ilt.iter().enumerate() {
                    ...
                    match lookup {
                        goblin::pe::import::SyntheticImportLookupTableEntry::HintNameTableRVA(
                            (_, ref hint_entry),
                        ) => {
                ...
                let Some(iat_off) = rva_to_off(iat_rva) else {
                    continue;
                };
                let mut i = 0usize;
                loop {
                    let entry_off = iat_off + i * ptr_size;
                    if entry_off + ptr_size > binary.len() {
                        break;
                    }
                    let raw_entry = if ptr_size == 4 {
                        u32::from_le_bytes(
                            binary[entry_off..entry_off + 4]
                                .try_into()
                                .unwrap_or([0; 4]),
                        ) as u64
                    } else {
                        u64::from_le_bytes(
                            binary[entry_off..entry_off + 8]
                                .try_into()
                                .unwrap_or([0; 8]),
                        )
                    };
...
fn resolve_pe_manual(binary: &[u8], map: &mut HashMap<u64, String>) {
    ...
    if pe_off + 24 > binary.len() || &binary[pe_off..pe_off + 4] != b"PE\0\0" {
        return;
    }
    ...
    // Parse section table
    let num_sec =
        u16::from_le_bytes(binary[pe_off + 6..pe_off + 8].try_into().unwrap_or([0; 2])) as usize;
    let opt_hdr_size = u16::from_le_bytes(
        binary[pe_off + 20..pe_off + 22]
            .try_into()
            .unwrap_or([0; 2]),
    ) as usize;
    ...
    if opt_off + 108 > binary.len() {
        return;
    }
    let import_rva = u32::from_le_bytes(
        binary[opt_off + 104..opt_off + 108]
            .try_into()
            .unwrap_or([0; 4]),
    ) as u64;
    if import_rva == 0 {
    ...
    let Some(imp_off) = rva_to_off(import_rva) else {
        return;
    };
    ...
    for i in 0..100 {
        ...
        if off + 20 > binary.len() {
            break;
        }
        let ilt_rva = u32::from_le_bytes(binary[off..off + 4].try_into().unwrap_or([0; 4])) as u64;
        let name_rva =
            u32::from_le_bytes(binary[off + 12..off + 16].try_into().unwrap_or([0; 4])) as u64;
        let iat_rva =
            u32::from_le_bytes(binary[off + 16..off + 20].try_into().unwrap_or([0; 4])) as u64;
        if ilt_rva == 0 && name_rva == 0 {
            break;
        }

        // Skip descriptors with invalid name RVA (anti-analysis technique)
        let Some(name_off) = rva_to_off(name_rva) else {
            continue;
        };
        if name_off >= binary.len() {
            continue;
        ...
        let Some(source_off) = rva_to_off(source_rva) else {
            continue;
        };
