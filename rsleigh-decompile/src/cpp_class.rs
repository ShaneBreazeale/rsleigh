...
pub fn recover_msvc_classes(binary: &[u8]) -> Vec<CppClass> {
    ...
    for i in (0..rdata_size.saturating_sub(col_size)).step_by(4) {
        ...
        let td_ref =
            u32::from_le_bytes(binary[fo + 12..fo + 16].try_into().unwrap_or([0; 4])) as u64;
        let td_rva = if is_64 {
            td_ref
        } else {
            td_ref.checked_sub(base).unwrap_or(td_ref)
        };
        let Some(td_off) = rva_to_off(td_rva) else {
            continue;
        };

        // Validate TypeDescriptor: first field is pVFTable (should point to .rdata)
        if td_off + ptr_size * 2 + 4 > binary.len() {
            continue;
        }
        ...
        let chd_ref =
            u32::from_le_bytes(binary[fo + 16..fo + 20].try_into().unwrap_or([0; 4])) as u64;
        let chd_rva = if is_64 {
            chd_ref
        } else {
            chd_ref.checked_sub(base).unwrap_or(chd_ref)
        };
        ...
        if let Some(chd_off) = rva_to_off(chd_rva) {
            if chd_off + 16 <= binary.len() {
                let num_bases = u32::from_le_bytes(
                    binary[chd_off + 8..chd_off + 12]
                        .try_into()
                        .unwrap_or([0; 4]),
                ) as usize;
                let bca_ref = u32::from_le_bytes(
                    binary[chd_off + 12..chd_off + 16]
                        .try_into()
                        .unwrap_or([0; 4]),
                ) as u64;
                let bca_rva = if is_64 {
                    bca_ref
                } else {
                    bca_ref.checked_sub(base).unwrap_or(bca_ref)
                };

                if let Some(bca_off) = rva_to_off(bca_rva) {
                    // BaseClassArray: array of RVAs/VAs to BaseClassDescriptors
                    for j in 1..num_bases.min(20) {
                        // skip index 0 (self)
                        let entry_off = bca_off + j * 4;
                        if entry_off + 4 > binary.len() {
                            break;
                        }
                        let bcd_ref = u32::from_le_bytes(
                            binary[entry_off..entry_off + 4]
                                .try_into()
                                .unwrap_or([0; 4]),
                        ) as u64;
                        let bcd_rva = if is_64 {
                            bcd_ref
                        } else {
                            bcd_ref.checked_sub(base).unwrap_or(bcd_ref)
                        };
                        if let Some(bcd_off) = rva_to_off(bcd_rva) {
                            // BCD: TypeDescriptor_ref at offset 0
                            if bcd_off + 4 <= binary.len() {
                                let base_td_ref = u32::from_le_bytes(
                                    binary[bcd_off..bcd_off + 4].try_into().unwrap_or([0; 4]),
                                ) as u64;
                                let base_td_rva = if is_64 {
                                    base_td_ref
                                } else {
                                    base_td_ref.checked_sub(base).unwrap_or(base_td_ref)
                                };
                                if let Some(base_td_off) = rva_to_off(base_td_rva) {
                                    let base_name_off = base_td_off + ptr_size * 2;
                                    if base_name_off < binary.len() {
                                        let end = binary[base_name_off..]
                                            .iter()
                                            .position(|&b| b == 0)
                                            .unwrap_or(0);
                                        if end > 0 && end < 256 {
                                            if let Ok(base_mangled) = std::str::from_utf8(
                                                &binary[base_name_off..base_name_off + end],
                                            ) {
                                                let base_clean = base_mangled
                                                    .strip_prefix(".?AV")
                                                    .or_else(|| base_mangled.strip_prefix(".?AU"))
                                                    .unwrap_or(base_mangled)
                                                    .trim_end_matches('@');
                                                let bp: Vec<&str> = base_clean
                                                    .split('@')
                                                    .filter(|s| !s.is_empty())
                                                    .collect();
                                                let base_name = if bp.len() > 1 {
                                                    let mut r = bp;
                                                    r.reverse();
                                                    r.join("::")
                                                } else {
        ...
        for k in (0..rdata_size.saturating_sub(ptr_size)).step_by(ptr_size) {
            ...
            if ptr_off + ptr_size > binary.len() {
                break;
            }
            let ptr_val = if is_64 {
                let raw =
                    u64::from_le_bytes(binary[ptr_off..ptr_off + 8].try_into().unwrap_or([0; 8]));
                if raw > base {
                    raw - base
                } else {
                    raw
                ...
                while vt_off + ptr_size <= binary.len() {
                    let method_rva = if is_64 {
                        let raw = u64::from_le_bytes(
                            binary[vt_off..vt_off + 8].try_into().unwrap_or([0; 8]),
                        );
                        if raw > base {
                            raw - base
                        } else {
                        ...
                        u32::from_le_bytes(binary[vt_off..vt_off + 4].try_into().unwrap_or([0; 4]))
                            as u64
                    };
        ...
        let size_estimate = if !virtual_methods.is_empty() {
            ptr_size as u64
        } else {
            0
...
pub fn recover_gcc_classes(binary: &[u8]) -> Vec<CppClass> {
    ...
    for sym in &all_syms {
        ...
        if let Some(fo) = vtable_fo {
            ...
            let max_entries = if vtable_size > ptr_size * 2 {
                (vtable_size - ptr_size * 2) / ptr_size
            } else {
                20
            };

            // Find .text bounds for validation
            let text_bounds: Vec<(u64, u64)> = elf
                .section_headers
                .iter()
            ...
            for idx in 0..max_entries.min(50) {
                ...
                if method_addr == 0 || !in_text(method_addr) {
                    break;
                }

                // Try to find a symbol name for this method
                let method_name = all_syms
                    .iter()
                    .find(|s| {
                        s.st_value == method_addr && s.st_type() == goblin::elf::sym::STT_FUNC
                    })
                    .and_then(|s| strtab(s))
                    .map(|n| {
                        // Try to demangle
                    ...
                    if let Some(pos) = t.find("->field_") {
                        ...
                        if hex_end > hex_start {
                            if let Ok(offset) = u64::from_str_radix(&t[hex_start..hex_end], 16) {
                                ...
                                let inferred = if t.contains("strcmp")
                                    || t.contains("strlen")
                                    || t.contains("printf")
                                {
                                    "char *"
    ...
    for class in classes {
        ...
        if class.base_classes.is_empty() {
            ...
            out.push_str(&format!(
                "class {} : public {} {{\n",
                class.name,
                class.base_classes.join(", public ")
            ));
        }
        out.push_str("public:\n");
