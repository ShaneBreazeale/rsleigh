...
fn emit_region(
    ...
    loop {
        ...
        let is_self_loop = is_loop_header
        ...
        match &block.terminator {
                ...
                if is_loop_header {
                    ...
                    if let Some(back_src) = back_source {
                        if back_src.0 < ssa.blocks.len() {
                            ...
                            let latch_block = if matches!(
                                &back_block.terminator,
                                SsaTerminator::CBranch { .. }
                            ) {
                                Some(back_src)
                            } else if let SsaTerminator::Fallthrough(next)
                            | SsaTerminator::Branch(next) = &back_block.terminator
                            {
                                if matches!(
                                    &ssa.blocks[next.0].terminator,
                                    SsaTerminator::CBranch { .. }
                                ) {
                                    Some(*next)
                                } else {
                            ...
                            if let Some(latch) = latch_block {
                                ...
                                } = &ssa.blocks[latch.0].terminator
                                {
                                    ...
                                    if (*taken == current || *fallthrough == current)
                                        && exit.0 < cfg.blocks.len()
                                    {
                                        ...
                                        let exit_was_emitted = if exit.0 < emitted.len() {
                                            emitted[exit.0]
                                        } else {
                                            true
                                        };
                                        if exit.0 < emitted.len() {
                                            emitted[exit.0] = true;
                                        }
                    ...
                    } = &next_block.terminator
                    {
                        if !emitted[next.0] {
                            ...
                            let (body_start, exit, negate) =
                                if can_reach(cfg, *taken, current, emitted)
            ...
            SsaTerminator::CBranch {
                    ...
                    if let Some(back_src) = back_source {
                        if back_src != current && back_src.0 < ssa.blocks.len() {
                            ...
                            } = &latch.terminator
                            {
                                ...
                                if (*latch_taken == current
                                    || *latch_fall == current
                                    || *latch_taken == latch_to_continue
                                    || *latch_fall == latch_to_continue)
                                    && exit.0 < cfg.blocks.len()
                                {
                                    let mut body = Vec::new();
                        ...
                        for stmt in &block.stmts {
                            if let Stmt::Assign(var_id) = stmt {
                                if matches!(&ssa.vars[var_id.0 as usize].expr, Expr::Phi(_)) {
                                    ...
                                    out.push(StructuredStmt::Assign {
                                        lhs: *var_id,
                                        rhs: *var_id,
                                    });
                                } else {
                                    // Emit non-Phi stmts inside the while body
                                    body.push(StructuredStmt::Assign {
                                        lhs: *var_id,
                                        rhs: *var_id,
                                    });
                                }
                                ...
                                match stmt {
                                    Stmt::Store { addr, val } => {
                                        body.push(StructuredStmt::Store {
                                            addr: *addr,
                                            val: *val,
                                        });
                                    }
                ...
                if merge_valid {
                    emitted[merge.0] = merge_was_emitted;
                }
                    ...
                    if let Some(back_src) = back_source {
                        if back_src.0 < ssa.blocks.len() {
                            ...
                            } = &back_block.terminator
                            {
                                ...
                                if (*taken == current || *back_fall == current)
                                    && exit.0 < cfg.blocks.len()
                                {
...
fn flatten_if_return(stmts: &mut Vec<StructuredStmt>) {
    ...
    while i < stmts.len() {
        ...
        // → if (cond) { ...; return; } BODY
        if let StructuredStmt::IfElse {
            then_body,
            else_body,
            ..
        } = &stmts[i]
        {
            let then_ends_with_exit = matches!(
                then_body.last(),
                Some(StructuredStmt::Return(_))
                    | Some(StructuredStmt::Break)
                    | Some(StructuredStmt::Continue)
                    | Some(StructuredStmt::Goto(_))
            );
    ...
    while i < stmts.len() {
        ...
        if let StructuredStmt::IfElse { cond, .. } = &stmts[i] {
            if let Some((test_var, _, _)) = extract_eq_const(*cond, ssa) {
                ...
                let collected =
                    collect_switch_cases(&stmts[i], test_var, ssa, &mut cases, &mut default);
    ...
    } = stmt
    {
        if let Some((var, val, is_eq)) = extract_eq_const(*cond, ssa) {
            if same_test_var(var, test_var, ssa) {
                ...
                let next_if = chain_body
                    .iter()
                    .enumerate()
                    .find(|(_, s)| matches!(s, StructuredStmt::IfElse { .. }));
                    ...
                    } = next_stmt
                    {
                        if let Some((next_var, _, _)) = extract_eq_const(*next_cond, ssa) {
                            if same_test_var(next_var, test_var, ssa) {
                                return collect_switch_cases(
                                    next_stmt, test_var, ssa, cases, default,
                                );
                            }
                        }
                    }
...
fn eliminate_gotos_inner(
    stmts: &mut Vec<StructuredStmt>,
    ssa: &SsaCfg,
    cfg: &Cfg,
...
fn can_reach_limited(cfg: &Cfg, from: BlockId, target: BlockId, limit: usize) -> bool {
    ...
    while let Some(node) = stack.pop() {
        ...
        steps += 1;
        if node.0 >= cfg.blocks.len() || visited[node.0] {
            continue;
        }
        visited[node.0] = true;
