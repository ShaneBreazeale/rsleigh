...
- Prefer bounded cards first, then explicit JSON/NDJSON artifacts when needed.
  P-code/SSA dumps are not capped like cards. They include typed operations or
  expressions alongside readable debug strings; save full dumps to files.
...
- Use `rsleigh FILE --index DIR` for a reusable navigation map. Add
  `--analysis-cache DIR` to cards and slices to reuse decoded/SSA snapshots.
...
- Prefer `--ssa-slice FUNCTION --return`, `--call-site ADDR --arg INDEX`, or
  `--condition ADDR` for a dependency question. Use one selector; add `--at ADDR`
  to disambiguate returns. Argument slots are zero-based. `--var ID` requires
  the matching SSA snapshot.
- Slices follow supported exact memory stores and helper argument/return
  bindings. MIPS/RISC-V argument and frame-memory conventions remain unsupported.
  Unknown memory/calls and truncated slices stay unresolved; dependence does
  not prove reachability. Preserve `(context_id, var_id)` when joining nodes.
- Preserve `evidence.functions` snapshot identities and raw origins. Current
  schemas are `rsleigh.card/v2`, `rsleigh.pcode/v2`, and `rsleigh.ssa-slice/v3`;
  an operation's index refers to `raw-pcode/v1`, before optimization.
- Output caps alone do not bound runtime. Cards and slices support
  `--max-decode-instructions N`, `--max-ssa-work N`, and cooperative
  `--deadline-ms N`. Slices also bound nodes, depth, call depth, functions, and
  traversal work. Inspect stop records and cache/work metrics.
...
For a value question, select the relevant return, call argument, or condition:

```bash
rsleigh FILE --ssa-slice ADDR --return --analysis-cache analysis-cache/
rsleigh FILE --ssa-slice ADDR --call-site CALL_ADDR --arg 0 --analysis-cache analysis-cache/
```

For a reconstruction, only if the P-code is coherent, run Round 3:
