...
fn main() {
    let t = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            while io < 128 && off + io < data.len() {
                match dec.decode(&data[off + io..], addr + io as u64) {
                    Ok(inst) => {
                        ...
                        if len == 0 { break; }
                        let is_ret = inst.disassembly.contains("ret") || inst.disassembly.contains("RET");
            ...
            println!("fibonacci: {} blocks, {} vars", ssa.blocks.len(), ssa.vars.len());
            for (bi, block) in ssa.blocks.iter().enumerate() {
                let term = match &block.terminator {
                    rsleigh_decompile::ir::SsaTerminator::Return(None) => "Return(None)".to_string(),
                    rsleigh_decompile::ir::SsaTerminator::Return(Some(v)) => {
                        let vd = &ssa.vars[v.0 as usize];
                        format!("Return(VarId({}) off={} expr={:?})", v.0, vd.varnode.offset,
                            match &vd.expr { rsleigh_decompile::ir::Expr::Unknown => "Unk", _ => "other" })
                    }
                    rsleigh_decompile::ir::SsaTerminator::Branch(t) => format!("Branch({})", t.0),
                    rsleigh_decompile::ir::SsaTerminator::CBranch { taken, fallthrough, .. } =>
