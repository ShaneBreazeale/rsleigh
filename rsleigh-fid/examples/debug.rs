...
use rsleigh_fid::{FidDb, ingest::fingerprint};
...
fn main() {
    ...
    let va_to_off = |va, sz| -> Option<(usize, usize)> {
        for (s, e, o, f) in &segs {
            if va >= *s && va + sz <= *e { let d = va - s; if d+sz > *f { return None; } return Some(((o+d) as usize, sz as usize)); }
        }
        None
    };
        ...
        if let Some(hq) = fingerprint(Architecture::X86_64, body, va, |_| None) {
            ...
            if m.len() > 1 {
                ...
                if distinct.len() > 1 {
                    multi_examples += 1;
                    if multi_examples <= 3 {
                        println!("QUERY: {} ({} bytes, {} units)", name, hq.body_len, hq.code_units);
                        println!("  matches ({}):", m.len());
                        for n in distinct.iter().take(5) { println!("    {}", n); }
                    }
                }
            }
            if multi_examples >= 3 { break; }
        }
    }
